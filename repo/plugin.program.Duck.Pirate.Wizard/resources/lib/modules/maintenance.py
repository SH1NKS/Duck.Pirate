...
def advanced_settings():
	selection = xbmcgui.Dialog().select(local_string(30038), ['1GB (1st - 3rd gen, Lite Firestick)','1GB (With Simple Settings)','1GB to 1.5GB (4k Firestick)','1GB to 1.5GB (With Simple Settings)','1.5GB to 2GB (Firebox, Cube, Sheild Tube)','1.5GB to 2GB (With Simple Settings)','2GB to 3GB RAM','2GB to 3GB (With Simple Settings)','3GB or more (Nvidia Shield Pro)','3GB or more (With Simple Settings)',local_string(30039)])  # Select Ram Size, Delete
	if selection==0:
		xml = os.path.join(advancedsettings_folder, 'less1.xml')
	elif selection==1:
		xml = os.path.join(advancedsettings_folder, 'less1.simple.xml')
	elif selection==2:
		xml = os.path.join(advancedsettings_folder, '1plus.xml')
	elif selection==3:
		xml = os.path.join(advancedsettings_folder, '1plus.simple.xml')
	elif selection==4:
		xml = os.path.join(advancedsettings_folder, 'firetv.xml')
	elif selection==5:
		xml = os.path.join(advancedsettings_folder, 'firetv.simple.xml')
	elif selection==6:
		xml = os.path.join(advancedsettings_folder, '2plus.xml')
	elif selection==7:
		xml = os.path.join(advancedsettings_folder, '2plus.simple.xml')
	elif selection==8:
		xml = os.path.join(advancedsettings_folder, 'shield.xml')
	elif selection==9:
		xml = os.path.join(advancedsettings_folder,'shield.simple.xml')
	elif selection==10:
